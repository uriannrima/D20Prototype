...
public class FogOfWar : BaseManager<FogOfWar>
{
    Mesh mesh = null;
    ...
    void copyHeight()
    {
        if (heightSampler != null)
        {
            ...
            for (int fRow = planeTopLeftRow; pRow < vertCount; ++fRow, ++pRow)
            {
                ...
                {
                    int planeIndex = (pRow * vertCount) + pCol;
                    int fogIndex = (fRow * nodeCount) + fCol;

                    if (planeIndex < 0 || planeIndex >= planeVertices.Length || fogIndex < 0 || fogIndex >= fogHeight.Length) continue;

                    planeVertices[planeIndex].y = fogHeight[fogIndex];
                }
            }
    ...
    void copyData()
    {
        ...
        for (int fRow = planeTopLeftRow; pRow < vertCount; ++fRow, ++pRow)
        {
            ...
            for (int fCol = planeTopLeftCol; pCol < vertCount; ++fCol, ++pCol)
            {
                ...
                planeData[planeIndex] = fogData[fogIndex];
            }
        }
